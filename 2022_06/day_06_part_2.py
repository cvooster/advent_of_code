"""This module determines the character of the first start-of-message marker."""
...
EXAMPLE_ANSWERS = [19, 23, 23, 29, 26]
...
def main():
    ...
    print(f"\nThe first start-of-message marker arrives at {nr_characters}.")


def get_first_start_marker(filename):
    """Read the signal, and locate the first start-of-message marker."""
    with open(filename) as file_object:
        signal = file_object.read().rstrip()
    if len(signal) < PATTERN_LENGTH:
        sys.exit("No start-of-message marker found: signal too short!")

    for i in range(PATTERN_LENGTH, len(signal)):
        slice = signal[i - PATTERN_LENGTH : i]
        if len(slice) == len(set(slice)):
            return i
    sys.exit("No start-of-message marker found!")
